...
class OldSdpc:
    ...
    def crop_patches(self, tile_size, overlap_size, patch_level, save_dir, blank_TH=0.7, is_mark=True,
            ...
            thumbnail = np.array(self.read_region((0, 0), thumbnail_level, self.level_dimensions[thumbnail_level]).convert('RGB'))
            ...
            tile_x = int(tile_size / pow(zoom_value,
                                         thumbnail_level - patch_level))  # Cut 1000*1000 (20x) when patch_level = 1. Thumbnail is done when patch_level = 2.
            tile_y = int(tile_size / pow(zoom_value, thumbnail_level - patch_level))  # Used to find relevant positions in the scaled image
            ...
            for i in range(int(np.floor((thumbnail_x - tile_x) / (tile_x - x_overlap) + 1))):
                for j in range(int(np.floor((thumbnail_y - tile_y) / (tile_y - y_overlap) + 1))):
                    ...
                    if np.sum(mask == 0) / mask.size < blank_TH:
                        ...
                        img_np = np.array(self.read_region((cut_x, cut_y), patch_level, (tile_size, tile_size)).convert('RGB'))  # numpy format
...
def OpenSdpc(wsi_path):
    extension = wsi_path.split('.')[-1].lower()
    if extension == 'sdpc':

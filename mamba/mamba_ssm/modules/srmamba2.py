...
from mamba.mamba_ssm.ops.selective_scan_interface2 import mamba_split_conv1d_scan_combined_no_out_proj
...
class SRMamba2(nn.Module):
    def __init__(
        ...
        A_b_log = torch.log(A).to(dtype=dtype)
    ...
    def forward(self, u, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None, rate=10):
        """
        ...
        if self.use_mem_eff_path and inference_params is None:
            ...
            N, C, L = zxbcdt.shape
            zxbcdt_b = TransposeTokenReEmbedding.transpose_normal_padding(zxbcdt, rate=rate)
            ...
            d_mlp_b = (zxbcdt_b.shape[-1] - 2 * self.d_ssm - 2 * self.ngroups * self.d_state - self.nheads) // 2
            z0_b, x0_b, z_b, xBC_b, dt_b = torch.split(
                zxbcdt_b,
                [d_mlp_b, d_mlp_b, self.d_ssm, self.d_ssm + 2 * self.ngroups * self.d_state, self.nheads],
                dim=-1
            )
            ...
            if causal_conv1d_fn is None or self.activation not in ["silu", "swish"]:
                ...
                xBC_b = self.act(
                    self.conv1d(xBC_b.transpose(1, 2)).transpose(1, 2)[:, -(self.dconv - 1):]
                )  
            else:
                assert self.activation in ["silu", "swish"]
            ...
            x_b, B_b, C_b = torch.split(xBC_b, [self.d_ssm, self.ngroups * self.d_state, self.ngroups * self.d_state], dim=-1)
            ...
            if d_mlp > 0:
                ...
                y_b = torch.cat([F.silu(z0) * x0, y_b], dim=-1)
            if seqlen_og is not None:

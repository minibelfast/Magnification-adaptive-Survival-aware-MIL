...
class WholeSlideImage(object):
    def __init__(self, path):
        ...
        print(path)
        if path.endswith('.svs') or path.endswith('.ndpi'):
            self.wsi = openslide.open_slide(path)
            self.magnification = self._read_magnification(self.wsi.properties)
            self.level_downsamples = self._assertLevelDownsamples()
        elif path.endswith('.sdpc'):
            self.wsi=opensdpc.OpenSdpc(path)
            #print(dir(self.wsi))
            self.magnification=self.wsi.scan_magnification
            #print(self.wsi.scan_magnification)
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        if isinstance(scale, (tuple, list)):
            patch_downsample = (int(scale[0]), int(scale[1]))
        else:
            # 如果是浮点数，使用相同的值作为x和y的下采样率
            patch_downsample = (int(scale), int(scale))
    ...
    def block_blending(self, img, vis_level, top_left, bot_right, alpha=0.5, blank_canvas=False, block_size=1024):
        ...
        # 处理不同格式的 downsample
        if isinstance(downsample, (tuple, list)):
            downsample_x = int(downsample[0])
            downsample_y = int(downsample[1])
        else:
            # 如果是浮点数，使用相同的下采样率
            downsample_x = downsample_y = int(downsample)
    
        shift = top_left # amount shifted w.r.t. (0,0)
        for x_start in range(top_left[0], bot_right[0], block_size_x * downsample_x):
            for y_start in range(top_left[1], bot_right[1], block_size_y * downsample_y):
                # 1. convert wsi coordinates to image coordinates via shift and scale
                x_start_img = int((x_start - shift[0]) / downsample_x)
                y_start_img = int((y_start - shift[1]) / downsample_y)

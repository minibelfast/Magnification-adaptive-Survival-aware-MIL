...
class MambaAttn(nn.Module):
    def __init__(self,num_classes=1,dropout=0,act='ReLU',in_dim = 512,survival = False, layer=2, rate=10, type="SRMamba"):
        ...
        bag_classifiers = [nn.Linear(512, 4) for i in range(4)] #use an indepdent linear layer to predict each class
    ...
    def forward(self, x, return_attn=False,no_norm=False):
       #feature = self.feature(x)
        ...
        #A = self.PVMamba(feature)
        if self.type == "SRMamba":
            for layer in self.layers:
                A_ = feature
                A = layer[0](feature)
                A = layer[1](A, rate=self.rate)
                A = A + A_
        ...
        #A = A.squeeze(0)
        #class token#transformer class token
        
        #size.(token,512)

        #B = feature.permute(0, 2, 1)
        #size.(1,1,512,token)
        ...
        for layer in self.layers2:
            ...
            B = B + B_
        B = self.normA(B)
        #B = self.WTConv2d(feature) + feature#分别输入全局空间注意（GSA）模块和局部空间注意力 （LSA） 模块
        #B = self.normB(B.squeeze(-1).permute(0, 2, 1))
        
        feature = self.DFF(A.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1),B.permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)).squeeze(-1)#给特征加权重
        ...
        A = A.squeeze(0)
        feature = torch.mm(A, feature) # [B, K, 512]

        logits = torch.empty(1, 4).float().to(feature.device)
        for c in range(4):
            logits[0, c] = self.classifiers[c](feature[c])

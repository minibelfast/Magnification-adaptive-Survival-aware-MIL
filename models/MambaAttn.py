...
class MambaAttn(nn.Module):
    ...
    def forward(self, x, return_attn=False,no_norm=False):
       #feature = self.feature(x)
        ...
        #print(A.shape)#torch.Size([1, 1, 52805])
        #print(A_raw.shape)#torch.Size([1, 52805, 1])
        #print(feature.shape)#torch.Size([1, 52805, 512])
        ...
        #print(feature.shape)#torch.Size([1, 512])
        #print(logits.shape)#torch.Size([1, 4])
        #print(Y_prob.shape)#torch.Size([1, 4])
        #print(Y_hat.shape)#torch.Size([1, 1])
        #raise ValueError("已通过")
        A_raw = A_raw
        results_dict = None
        if self.survival:

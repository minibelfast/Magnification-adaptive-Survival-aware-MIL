...
def has_UNI():
    HAS_UNI = False
    UNI_CKPT_PATH = str(Path(__file__).resolve().parents[1] / "UNI" / "pytorch_model.bin")
    try:
        candidates = []
        env_path = os.environ.get("UNI_CKPT_PATH")
        if env_path:
            candidates.append(env_path)
        root = Path(__file__).resolve().parents[1]
        candidates.append(str(root / "UNI" / "pytorch_model.bin"))

        ckpt_path = None
        for c in candidates:
            p = Path(c)
            if p.is_dir():
                p2 = p / "pytorch_model.bin"
                if p2.is_file():
                    ckpt_path = str(p2)
                    break
            elif p.is_file():
        ...
        if not ckpt_path:
            raise ValueError("UNI checkpoint not found")
        HAS_UNI = True
